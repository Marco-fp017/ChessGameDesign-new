...
public class UI {
    ...
    public static ChessPosition readChessPosition(Scanner sc){
        try{
            ...
            char readColumn = ax.charAt(0);
            int readRow = (int) Integer.parseInt(ax.substring(1));
    ...
    public static void printMatch(ChessMatch ch, List<ChessPiece> cp){
    ...
    public static void printBoard(ChessPiece[][] ch, boolean[][] possibleMoves) {
        for (int i = 0; i < ch.length ; i++){
            System.out.print((8 - i) + " ");
            for (int j = 0; j < ch.length ; j++){
                printPiece(ch[i][j], possibleMoves[i][j]);
            }
            System.out.println();
        }
        System.out.println("  a b c d e f g h");        
    ...
    if(possibleMove) System.out.print(ANSI_BLUE_BACKGROUND);
